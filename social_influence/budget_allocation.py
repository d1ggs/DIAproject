...
class StatelessBudgetAllocation(object):
    ...
    def initialization_step_joint_influence(self, social_list):
        """
        Initialization of the algorithm.
        Pre-computes social influence for the first 2 steps for each social and returns a dictionary containing the tuples
        """
        ...
        # Calculates first two steps of influence for each social
        for i in range(3):
            best_node1, influence1 = social_list[i].step_fit(self.mc_simulations, self.n_steps_montecarlo)
            results[i].append((best_node1, influence1))
            best_node2, influence2 = social_list[i].step_fit(self.mc_simulations, self.n_steps_montecarlo, resume_seeds=[best_node1])
            results[i].append((best_node2, influence2))
    ...
    def joint_influence_maximization(self, matrix1, matrix2, matrix3,weights=None):
        ...
        social_list = [social1_learner, social2_learner, social3_learner]
        cumulative_influence1, cumulative_influence2, cumulative_influence3 = self.initialization_step_joint_influence(social_list)
        cumulative_influences = [cumulative_influence1, cumulative_influence2, cumulative_influence3]
        if self.verbose:
            print(cumulative_influences[0], cumulative_influences[1], cumulative_influences[2])
        if weights is None:
        ...
        while not sum(budget) == self.budget_total:
            ...
            if sum(budget) != self.budget_total:
                ...
                # Computes the next step to use in the next iteration
                best_node, influence = social_list[argument_to_increment].step_fit(self.mc_simulations,
                                                                                        self.n_steps_montecarlo,
                                                                                        resume_seeds=resume_seeds)
                cumulative_influences[argument_to_increment][budget[argument_to_increment] + 1] = [best_node, influence]
