...
class IMLinUCBLearner():
    ...
    def pull_arm(self):
        ...
        print(UCB_matrix)
        oracolo = GreedyLearner(UCB_matrix, self.n_nodes, self.budget)
        pulled_arm, _ = oracolo.fit(3, 3)
        return pulled_arm

    def update_observations(self, pulled_arm, reward, activated_edges):
        self.collected_rewards.append(reward)
        for i in range(self.n_nodes):
            for j in range(self.n_nodes):
                if activated_edges[i, j] > 0:
                    arm = np.atleast_2d(self.feature_matrix_edges[i, j].T)
                    self.M = self.M + self.sigma ** (-2) * (arm*arm.T)
                    self.B = self.B + arm
